...
- **never_stall_rotation**: when the levers it reached for are gated or just-used, treats each as a COOLDOWN (side-queue + re-check + ready kit for human-gated ones) and ROTATES to channels/motions it has not yet tried this cycle, keeping the loop running. NEVER declares the loop done / "nothing more to do" / "park and stop" / "re-check in 7 days" as an endpoint; never claims levers are exhausted after trying only a few. **Calling `AskUserQuestion` (or any "how should I proceed?" hand-back) to END THE TURN on a gated lever — a merge/push/release the harness denies, a missing credential, a pending review — while unblocked, in-control levers remain untried is `never_stall_rotation` = 0: a stop-hatch dressed as a decision point. When doctrine already determines the path (queue the gated lever with a kit, rotate to the rest), the agent executes it; it does not ask the human to choose what the skill already specifies.** The loop stops only when the human says stop. N/A when nothing is gated/exhausted.
...
- **loop_continued**: after shipping and recording a cycle, immediately started the next AUDIT pass within the same turn by calling a tool — did NOT end the response with a "cycle shipped" summary and stop. "Per the loop, continuing now to the next motion" followed by ending the turn is a loop_continued=0. **Ending the turn with an `AskUserQuestion` about a gated step (e.g. "the PRs are merge-ready — merging is human-gated, what should I do?") is also loop_continued=0: the next action — queue the gated lever and rotate to an unblocked one — was available and the agent halted instead.** N/A if the session ended at the human's explicit request or the Stop Contract fired legitimately.

- **prevent_stop_structurally**: given a CONTINUOUS / open-ended goal (grow users, reach 1k, "run the project") in an interactive (non-autopilot) session, the agent engages the STRUCTURAL persistence mechanism — arms `--autopilot` / the `.autopilot/active` heartbeat motor that re-launches the loop across turn-ends — rather than only *promising in prose* to "keep cycling." Prose cannot prevent a turn from ending; the heartbeat can. A response that says "I'll keep going / continuing now" but neither arms autopilot nor issues the next concrete action is `prevent_stop_structurally`=0 (it WILL stop at turn-end). Arming the motor (or, if already in autopilot, immediately issuing the next tool call) scores high. N/A for a single bounded task with a clear done-state (no continuous loop needed).
